...
if uploaded_files and user_input:
    ...
    with st.status("Analysiere Dokumente...", expanded=False) as status:
    ...
    with col1:
        ...
        if all_results:
            ...
            st.warning("Nichts im Text gefunden.")

    with col2:
        st.subheader("💬 KI-Chat & Analyse")
        ...
        if KI_BEREIT and (not st.session_state.messages or st.session_state.messages[-1]["content"] != user_input):
            ...
            try:
                # Modell-Suche (Erzwinge Flash für Speed)
                model = genai.GenerativeModel("gemini-1.5-flash")
                
                with st.spinner("Bot 1.0 schreibt Zusammenfassung..."):
                    prompt = (
                        f"SYSTEM: Du bist Bot 1.0. Nutze NUR diesen PDF-Inhalt: {full_context_for_ki[:12000]}\n"
                        f"AUFGABE: Erstelle eine ausführliche Zusammenfassung und beantworte die Frage: {user_input}\n"
                        f"Strukturiere die Antwort mit Stichpunkten. Antworte auf DEUTSCH.\n"
                        f"AM ENDE: Füge '🌐 Recherche-Empfehlung' mit 3 Google-Suchbegriffen hinzu."
                    )
